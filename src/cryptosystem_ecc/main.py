...
from src.cryptosystem_ecc.unique_rabin_williams_ecc import (
    ECCUniqueRabinWilliamsDecryptor,
    ECCUniqueRabinWilliamsEncryptor,
    ECCUniqueRabinWilliamsKeyGenerator,
)
...
def main():
    ...
    if args.p and args.q and args.s and args.M:
        ...
        N, p, q, s = ECCUniqueRabinWilliamsKeyGenerator.generate_keys()
        M = 500000  # Example message
        print(f"Generated N: {N}, p: {p}, q: {q}, s: {s}")

    D = ECCUniqueRabinWilliamsEncryptor.encrypt(M, N)
    print(f"Encrypted message: {D}")

    decrypted_message = ECCUniqueRabinWilliamsDecryptor.decrypt(D, p, q)
